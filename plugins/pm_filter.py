...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, REQ_CHANNEL
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🗓 1/{math.ceil(int(total) / 10)}", callback_data="pages"),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>● Here is what i found for your query</b>\n● 𝙍𝙚𝙨𝙪𝙡𝙩 :  <code>{search}</code>\n● 𝙏𝙤𝙩𝙖𝙡 : <code>{total_results}</code>"
